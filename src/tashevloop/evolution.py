    ...
    for lesson in lessons:
        ...
        if failures >= 2 and failures >= successes:
            proposals.append({
                ...
                "title": lesson["title"],
                "reason": f"{failures} failure signals vs {successes} successful signals",
                "action": (
            ...
            proposals.append({
                "priority": "high",
                "kind": "unresolved-failure",
                "title": lesson["title"],
                "reason": "Failure evidence exists but no verified fix/success is recorded",
                "action": "Reproduce the problem, verify a fix, then record the successful outcome.",
                "signature": lesson["signature"],
            })
            ...
            proposals.append({
                ...
                "kind": "weak-lesson",
                "title": lesson["title"],
                "reason": f"{evidence} evidence items but only {confidence:.0%} confidence",
                "action": "Collect a stronger verification signal before relying on this lesson.",
                "signature": lesson["signature"],
            })
